...
class CardOperations: 
    ...
    async def add_one_card(cls,data:CardAddSchema):
        async with new_session() as session:
            query=select(accountModel).where(accountModel.id==data.account_id)
    ...
    async def update_card(cls,card_id,data:CardAddSchema):
        async with new_session() as session:
            ...
            if(data.balance<0):
                return {"ok":False,"error":"Incorrect value of balance"}
            query=update(cardModel).where(cardModel.id==card_id).values(account_id=data.account_id,balance=data.balance)
