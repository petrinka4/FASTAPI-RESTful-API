...
async def validateBranch(data: BaseModel, session: AsyncSession):
    return (await ValidateServise.validate_existence(cityModel, "city_id", data, session) and await ValidateServise.validate_existence(bankModel, "bank_id", data, session))
